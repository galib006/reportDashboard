...
function SingleOrderData({data}) {
    ...
    const [sngData,setSngData] = useState({matched:[],Ordertotal:0,Deliverytotal:0,Balancetotal:0})
     const apiData = cndata[0]?.apiData || [];
    
    useEffect(()=>{
        const matched = apiData.filter((dd)=>dd.WorkOrderNo == data)
        const Ordertotal = matched.reduce((acc,item)=> acc + Number(item.BreakDownQTY),0)
        const Deliverytotal = matched.reduce((acc,item)=> acc + Number(item.ChallanQTY),0)
