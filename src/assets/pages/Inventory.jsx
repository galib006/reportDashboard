...
function Inventory() {
  ...
  const dateSubmit = async (e) => {
    ...
    try {
      ...
      // material receive
      const openingReceiveMap = {
        "10 mm Single Satin (White)": 9,
        "10 mm Single Satin (White)": 11,
        ...
        "38 mm Single Satin Salvage (White)":0,
      ...
      const mtl = r3.map((it) => {
        const recs = r1
          .filter((d) => d.MaterialName === it.MaterialName)
        ...
        /////////////**********************************////////////////////////// */
        let events = [
          ...recs.map((r) => ({ ...r, type: "Receive" })),
          ...iss.map((q) => ({ ...q, type: "Issue" })),
        ];

        // ✅ opening receive add
        const openingQty = openingReceiveMap[it.MaterialName];

        if (openingQty) {
          events.unshift({
            type: "Receive",
            ActualReceiveQTY: openingQty,
            GRNDate: cndata.startDate || new Date(),
            GRNNo: "OPENING",
            VendorName: "Opening Balance",
            isOpening: true,
          });
        }

        // sort after adding opening
        events = events.sort(
          (a, b) =>
            new Date(a.GRNDate || a.IssueDate) -
            new Date(b.GRNDate || b.IssueDate),
        );

        /////////////**********************************////////////////////////// */
        events.forEach((ev) => {
          if (ev.type === "Receive")
            runningBalance += Number(ev.ActualReceiveQTY || 0);
        ...
        const totalRcv =
          recs.reduce((acc, c) => acc + Number(c.ActualReceiveQTY || 0), 0) +
          Number(openingQty || 0);
        const totalIss = iss.reduce(
          (acc, c) => acc + Number(c.IssueQTY || 0),
          0,
        );
  ...
  const processedList = useMemo(() => {
    ...
    if (filters.company !== "all")
      list = list.filter((it) =>
        it.timeline.some((x) => x.CompanyName === filters.company),
      );
    if (filters.category !== "all")
      list = list.filter((it) =>
        it.timeline.some((x) => x.CategoryName === filters.category),
      );
    if (filters.subcategory !== "all")
      list = list.filter((it) =>
        it.timeline.some((x) => x.SubCategoryName === filters.subcategory),
      );
    if (filters.material !== "all")
      list = list.filter((it) => it.itemName === filters.material);
    if (filters.search.trim() !== "")
      list = list.filter((it) =>
        it.itemName.toLowerCase().includes(filters.search.toLowerCase()),
      );
    if (filters.negativeGap)
      list = list.filter((it) => it.totalReceive - it.totalIssue < 0);
  ...
  const pagedList = filters.pageOff
    ? processedList
    : processedList.slice((page - 1) * pageSize, page * pageSize);
    ...
    processedList.forEach((it) => {
      ...
      it.timeline.forEach((ev) => {
        rows.push({
          ...
          IssueNo: ev.IssueNo,
          Remarks:
            ev.VendorName || ev.IssuedBy || ev.ChallanNo || ev.JobCardNo || "",
        });
      });
    });
  ...
  return (
    <div className="p-4">
      {/* Loading */}
      {loading ? (
        <div className="text-center font-bold py-2">Loading...</div>
      ) : (
        ""
      )}
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <label>
            <input
              ...
              onChange={(e) =>
                setFilters((f) => ({ ...f, negativeGap: e.target.checked }))
              }
            />{" "}
            Negative Gap
          </label>
        ...
        <div className="flex items-center gap-2">
          <label>
            <input
              type="checkbox"
              checked={filters.pageOff}
              onChange={(e) =>
                setFilters((f) => ({ ...f, pageOff: e.target.checked }))
              }
            />{" "}
          ...
          <table className="table w-full table-zebra border">
            ...
            <tbody>
              ...
              {pagedList.map((it, idx) => {
                let gap = 0;
              if (it.totalIssue > it.totalReceive) {
                gap = it.totalReceive - it.totalIssue; // negative value
              }
                return (
                  <React.Fragment key={it.itemName}>
                    <tr>
                      ...
                      <td>{Number(it.totalReceive).toFixed(2)}</td>
                      <td>{Number(it.totalIssue).toFixed(2)}</td>
                      <td>{Number(it.Balance).toFixed(2)}</td>
                      <td className={gap < 0 ? "text-red-600 font-bold" : ""}>
                              ...
                              <tbody>
                                {it.timeline.map((ev, i) => (
                                  ...
                                  >
                                    <td>{i + 1}</td>
                                    <td>
                                      {fmtDate(ev.GRNDate || ev.IssueDate)}
                                    </td>
                                    <td>
                                      {ev.GRNNo ||
                                        ev.IssueNo ||
                                    ...
                                    <td>
                                      {ev.type === "Issue" ? ev.IssueQTY : ""}
                                    </td>
                                    <td>{Number(ev.runningBalance).toFixed(2)}</td>
                                    <td>{ev.IssueNo}</td>
                                    <td>{ev.RequisitionNo}</td>
                                    <td>
                                      {ev.VendorName ||
