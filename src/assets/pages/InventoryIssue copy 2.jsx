...
UseData.map((row) => {
  ...
  reqTotal === 0
    ? "0.00"
    : ((pendingTotal / reqTotal) * 100).toFixed(2);
...
const range = XLSX.utils.decode_range(ws["!ref"]);
  ...
  ws[cell].s = {
    ...
    border: {
      ...
      const secReq = itemSummaryList.reduce(
  (s, r) => s + Number(r.Req || 0),
  0
);

const secIssue = itemSummaryList.reduce(
  (s, r) => s + Number(r.Issue || 0),
  0
);

const secPending = itemSummaryList.reduce(
  (s, r) => s + Number(r.Pending || 0),
  0
);

const secPendingPercent =
  secReq === 0
    ? "0%"
    : ((secPending / secReq) * 100).toFixed(2) + "%";

const totalRow = summaryStartRow + 2 + itemSummaryList.length;
...
for (let c = 0; c < 5; c++) {
  ...
  { wch: 15 }, // Issue Date
    ...
    const grandReq = masterRows.reduce(
  (s, r) => s + Number(r.Req || 0),
  0
);

const grandIssue = masterRows.reduce(
  (s, r) => s + Number(r.Issue || 0),
  0
);

const grandPending = masterRows.reduce(
  (s, r) => s + Number(r.Pending || 0),
  0
);

const grandPendingPercent =
  grandReq === 0
    ? "0%"
    : ((grandPending / grandReq) * 100).toFixed(2) + "%";

const grandRow = 3 + masterRows.length;
...
const existingSheets = wb.SheetNames.filter(
  (name) => name !== "Master Summary"
);

wb.SheetNames = ["Master Summary", ...existingSheets];
                    ...
                    <table className="min-w-full border border-gray-300">
                      ...
                      <tfoot className="bg-gray-100 font-semibold">
                        <tr>
                          ...
                          <td className="p-2 border">
                           {
                              item.Requisitions.reduce(
                                (reqSum, req) =>
                                  reqSum +
                                  req.Issues.reduce(
                                    (issSum, iss) => issSum + Number(iss.IssueQty || 0),
                                    0
                                  ),
                                0
                              )
